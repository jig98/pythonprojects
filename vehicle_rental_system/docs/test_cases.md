...
|---|-----------|-------|------------------|----------------|
| 21 | Log file is created automatically | Delete `data/` if present, then run `python main.py` and choose any option | `data/activity_log.json` is created, containing a valid JSON array | Created, valid JSON — Pass |
| 22 | Every operation is recorded | Run the full session from `docs/sample_terminal_session.txt` | 10 entries in the log, one per operation performed (including the blocked double-booking) | 10 entries, matching order — Pass |
| 23 | Log persists across separate runs | Run the program once, exit, run it again and perform one more action | Total entry count increases by exactly one; earlier entries are untouched | Confirmed (10 → 11) — Pass |
| 24 | Blocked attempts are logged with a reason | Try renting an already-rented vehicle | An entry with `"status": "blocked"` and `"details.reason": "vehicle_unavailable"` appears | Present — Pass |
| 25 | Dates serialize correctly | Return a vehicle late | The log entry's `return_date` field is a plain ISO string (e.g. `"2026-09-08"`), not a Python object or an error | Confirmed — Pass |

See `docs/sample_activity_log.json` for the exact log produced by that
recorded session.
