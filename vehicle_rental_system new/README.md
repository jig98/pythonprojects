...
|---|---|
| `RentalApp` (`cli/app.py`) | The terminal front-end: prints the menu, reads input, calls into `RentalService`, and turns any `RentalSystemError` into a friendly on-screen message. Contains no business logic itself. |
...
| **Polymorphism** | `RentalService` and `Rental` call `vehicle.calculate_rental_cost(days)` without knowing or checking the concrete subclass — each subclass's own override runs. See §7 below. |
...
## 7. Where polymorphism is used, and why it helps
